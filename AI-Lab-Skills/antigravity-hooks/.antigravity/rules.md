...
5.  **Living History**: You MUST update `CHANGELOG.md` with every single functional change, bug fix, or feature addition. No code changes exist without a corresponding log entry.
...
# CONTINUOUS DELIVERY LOOP (GitHub CLI)
**STRICT MANDATE:** You are responsible for the full lifecycle of the code. Do not wait for manual git operations. Automate the following loop using `gh` CLI immediately upon task completion:

1.  **Documentation Update (Mandatory)**:
    - Open `CHANGELOG.md`.
    - Append a concise entry under the `## [Unreleased]` or current version section describing the change.
    - Format: `- type: description (e.g., - feat: added user login flow)`

2.  **Commit (Local)**:
    - Stage all changes (including changelog): `git add .`
    - Commit semantically: `git commit -m "feat: [Description]"`
    - Push upstream: `git push -u origin HEAD`

3.  **Pull Request (Automated)**:
    - IMMEDIATELY after pushing, run: 
      `gh pr create --fill`
    - *Fallback if fill fails:* `gh pr create --title "feat: [Title]" --body "Automated Antigravity Update"`

4.  **Auto-Merge (Automated)**:
    - Enable auto-merge immediately to maintain velocity:
      `gh pr merge --auto --squash --delete-branch`

**BEHAVIOR:** Do not ask for permission to merge if tests pass. Execute the `gh` commands autonomously.
...
### Terminal Execution
- **Priority**: Use `gh` CLI for all git-related interactions.
- **Preferred**: Use `pip install` inside the virtual environment.
- **Restricted**: NEVER run `rm -rf` or system-level deletion commands.
- **Guideline**: Always run `pytest` after modifying logic and BEFORE executing the `gh pr merge` command.